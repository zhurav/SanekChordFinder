...
private:
    ...
    void processEnvelopeFrame() noexcept
    {
        ...
        const auto minimumFrames = static_cast<std::uint64_t>(std::ceil(6.0 * envelopeRate));
        if (!locked && onsetFrameCount >= minimumFrames
    ...
    void updateTempoEstimate() noexcept
    {
        const int available = std::min({ static_cast<int>(onsetFrameCount), historyCapacity,
                                         static_cast<int>(std::lround(12.0 * envelopeRate)) });
        float mean = 0.0f;
